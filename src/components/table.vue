                        ...
                        <!-- 自定义表格内容 -->
                            ...
                            <div v-else-if="item.type === 'btnsText'">
                                <el-button type="text" v-for="it in item.btnData"  @click="callBack(it.method,scope.row,scope.$index)" size="mini">
                                   <span v-if="it.color" :style="'color:'+it.color">{{it.name}}</span>
                                   <span v-else>{{it.name}}</span>
                                </el-button>
                            </div>
                            <!-- 下拉菜单 -->
                            <el-select v-else-if="item.type === 'select'" v-model="scope.row[item.field]" @change="selectChange(scope.row[item.field],scope.$index)"  clearable placeholder="请选择" size="mini">
                                <!-- select的数据与字段名相匹配 'select_'+'字段名' -->
                                <el-option v-if="item.props" v-for="_item in item.childrens" :label="_item[item.props.label]" :value="_item[item.props.value]">
                                </el-option>
                                <el-option v-else v-for="_item in item.childrens" :label="_item.label" :value="_item.value">
                                </el-option>
                            </el-select>
                            <el-input-number  controls-position="right" v-else-if="item.type === 'input-number'" v-model="scope.row[item.field]"  :min="0" size="mini" @change="numberChange(scope.row[item.field],scope.$index)"></el-input-number>
                            <!-- 未完待续 -->
                            </template>
                    </el-table-column>
            </el-table>
    ...
    methods:{
        ...
        selectChange(data,val){
            this.$emit('selectChange',data,val);
        },
        numberChange(data,val){
            if(data===0){
                return;
            }
            this.$emit('numberChange',data,val);
        }
    },
    watch:{
        'tableData':function(){
