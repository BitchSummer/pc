<template>
  <div class="tags_root" ref='tags_root'>
    <ul ref="tags_box" class="list_box" :style="'width:'+ (170* tags.length+20)+'px'">
      <li v-for="(item,i) in tags" @click.stop="clickTag(i)" :class="tagsActiveIndex==i? 'active':''"><i :class="i==0? 'qw_icon_home' : 'qw_icon_page'"></i>
        {{item.title}} <span v-if="i>0" @click.stop="closeTag(i)" class="el-icon-error"></span></li>
    </ul>
      ...
      div1.onmousedown = function (ev) {
        ...
        document.onmousemove = function (ev) {
          ...
          if (oevent.clientX - distanceX > 0) {
            div1.style.left = 0;
          } else if (Math.abs(oevent.clientX - distanceX) + parseInt(that.$refs['tags_root'].offsetWidth) - 106 -
            parseInt(div1.offsetWidth) > 0) {
            div1.style.left = -1 * (parseInt(div1.offsetWidth) - parseInt(that.$refs['tags_root'].offsetWidth) +
              106)
          } else {
